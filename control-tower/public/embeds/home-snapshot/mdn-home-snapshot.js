...
const matchCoverageLocation=data=>{const items=Array.isArray(data?.items)?data.items:[];const domainHost=value=>{try{const raw=String(value||"").trim();return new URL(/^https?:/i.test(raw)?raw:"https://"+raw).hostname.replace(/^www\./,"").toLowerCase()}catch{return""}};const exact=(keys)=>items.find(item=>keys.some(key=>domainHost(item?.[key])===host));const city=exact(["cityUrl","cityDomain"]);if(city)return{type:"city",name:first(city,["city","cityName"]),state:first(city,["state","stateName"]),county:first(city,["county","countyName"]),object:city};const state=exact(["stateUrl","stateDomain"]);if(state&&/^puerto rico$/i.test(first(state,["state","stateName"])))return{type:"state",name:first(state,["state","stateName"]),state:first(state,["state","stateName"]),object:state};const county=exact(["countyUrl","countyDomain"]);if(county)return{type:"county",name:first(county,["county","countyName"]),state:first(county,["state","stateName"]),county:first(county,["county","countyName"]),object:county};if(state)return{type:"state",name:first(state,["state","stateName"]),state:first(state,["state","stateName"]),object:state};return null};
