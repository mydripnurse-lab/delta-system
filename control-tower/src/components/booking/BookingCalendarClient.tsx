...
const emptyContact: Contact = { firstName: "", lastName: "", email: "", phone: "", phoneCountry: "US", dateOfBirth: "", weight: "", heightFeet: "", heightInches: "" };
...
function normalizedPerson(person: Contact) {
  const { phoneCountry: _phoneCountry, heightFeet: _heightFeet, heightInches: _heightInches, ...rest } = person;
  return {
    ...rest,
...
function PhoneField({
  ...
  return (
    <label className={styles.phoneField}>
      ...
      <small>{country.flag} {country.name} · dial code {country.dialCode} is added securely when booking.</small>
    </label>
  );
}

export function BookingCalendarClient({ publicKey, partnerId = "", partnerView = false }: { publicKey: string; partnerId?: string; partnerView?: boolean }) {
  ...
  const [sourceContext, setSourceContext] = useState<{ pageUrl: string; referrer: string; attribution: Record<string, string>; requestedPartnerId: string }>({ pageUrl: "", referrer: "", attribution: {}, requestedPartnerId: "" });
  ...
  useEffect(() => {
    ...
    setSourceContext({ pageUrl: originalPageUrl, referrer, attribution, requestedPartnerId });
    const initialPhone = params.get("phone") || "";
    const initialPhoneCountry = getPhoneCountry(initialPhone);
    setContact({
      firstName: (params.get("firstName") || params.get("fullName") || params.get("name") || "").trim().split(/\s+/)[0] || "",
      lastName: (params.get("lastName") || params.get("fullName") || params.get("name") || "").trim().split(/\s+/).slice(1).join(" "),
      email: params.get("email") || "",
      phone: formatPhone(initialPhone, initialPhoneCountry),
      phoneCountry: initialPhoneCountry,
      dateOfBirth: params.get("dateOfBirth") || params.get("dob") || "",
      weight: params.get("weight") || "",
      heightFeet: params.get("heightFeet") || (params.get("height") ? String(Math.floor(Number(params.get("height")) / 12)) : ""),
      heightInches: params.get("heightInches") || (params.get("height") ? String(Number(params.get("height")) % 12) : ""),
    });
    setAddress((current) => ({
      ...current,
      addressLine1: params.get("address") || params.get("addressLine1") || "",
    ...
    try {
      await fetch("/api/public/booking/leads", {
        ...
        body: JSON.stringify({
          ...
          attendees: additionalPatients.map(({ id: _id, ...patient }) => normalizedPerson(patient)),
  ...
  const contactIsComplete = useMemo(() => (
    Boolean(contact.firstName.trim() && contact.lastName.trim() && contact.email.trim() && contact.phone.trim() && contact.dateOfBirth && weightIsComplete(contact.weight) && heightIsComplete(contact))
  ), [contact]);

  const additionalPatientsAreComplete = useMemo(() => additionalPatients.every((patient) => (
    patient.firstName.trim() && patient.lastName.trim() && patient.email.trim() && patient.phone.trim() && patient.dateOfBirth && weightIsComplete(patient.weight) && heightIsComplete(patient)
  )), [additionalPatients]);
  ...
  const geocodeAddress = useCallback(async () => {
    ...
    const payload = await response.json() as { features?: Array<{ place_name?: string; text?: string; context?: Array<{ id?: string; text?: string }> }> };
    ...
    if (!feature || !county || !city || !state || !postalCode) throw new Error("We could not verify the county for that address. Check the street, city, state and ZIP code.");
    setAddress((current) => ({ ...current, county, city, state, postalCode }));
    return { county, city, state, postalCode };
  }, [address]);
  ...
  }, [address, canSearch, captureLead, date, geocodeAddress, partnerId, partnerView, publicKey, screeningIsClear]);

  const continueToLocation = useCallback(() => {
    if (!contactIsComplete) {
      setError("Enter the primary patient’s contact details, date of birth, weight, and height in feet and inches.");
      return;
    }
  ...
  const submitMedicalScreening = useCallback(() => {
    ...
    if (screeningSelected.length === 1 && screeningSelected[0] === "none") {
      setNotice("Screening complete. Enter the appointment location to see available times.");
    } else {
  ...
  }, [additionalPatients, additionalPatientsAreComplete, address, availability, contact, contactIsComplete, date, partnerId, publicKey, screeningIsClear, screeningSelected, selectedPartnerId, selectedSlot, sourceContext.requestedPartnerId]);
    ...
    <main className={styles.page}>
      <section className={styles.shell}>
        ...
        <div className={styles.layout}>
          <section className={`${styles.card} ${styles.screeningCard}`}>
          ...
          <section className={`${styles.card} ${styles.patientCard}`}>
            ...
            <h2 className={styles.sectionTitle}>Who is this appointment for?</h2>
            <p className={styles.sectionIntro}>Add the primary patient first. You can include other people who will receive care during the same visit.</p>
            ...
            <div className={styles.additionalHeader}>
              <div><strong>Additional patients</strong><small>Optional · every person needs complete contact information.</small></div>
              <button className={styles.secondaryButton} type="button" onClick={() => setAdditionalPatients((current) => [...current, newAdditionalPatient()])}>+ Add person</button>
            </div>
            {additionalPatients.map((patient, index) => (
              <div className={styles.additionalPatient} key={patient.id}>
                <div className={styles.additionalPatientHeader}><strong>Person {index + 2}</strong><button className={styles.removeButton} type="button" onClick={() => setAdditionalPatients((current) => current.filter((item) => item.id !== patient.id))}>Remove</button></div>
                <div className={styles.grid}>
          ...
          <section className={styles.card}>
            ...
            <div className={styles.grid}>
              <label className={styles.wide}>Street address<input value={address.addressLine1} onChange={(event) => setAddress((current) => ({ ...current, addressLine1: event.target.value }))} autoComplete="street-address" /></label>
