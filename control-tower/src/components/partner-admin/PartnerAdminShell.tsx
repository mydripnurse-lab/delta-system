...
const NAV_ITEMS = [
  { href: "/", label: "Applications", icon: "inbox" },
  { href: "/partners", label: "Partners", icon: "users" },
  { href: "/appointments", label: "Appointments", icon: "appointments" },
  { href: "/services", label: "Services", icon: "services" },
  { href: "/calendars", label: "Booking calendars", icon: "calendar" },
    ...
    <main className={styles.crmShell}>
      <aside className={`${styles.crmSidebar} ${menuOpen ? styles.crmSidebarOpen : ""}`}>
        <div className={styles.crmBrand}>
          <span className={styles.logo} aria-hidden="true">MDN</span>
          <div><strong>My Drip Nurse</strong><span>Partner Admin</span></div>
        ...
        <nav className={styles.crmNav} aria-label="Partner Admin">
          <span className={styles.crmNavLabel}>Workspace</span>
          {NAV_ITEMS.map((item, index) => {
            const active = item.href === "/"
              ? pathname === "/" || pathname === "/applications" || pathname.startsWith("/applications/")
              : pathname.startsWith(item.href);
            return (
              <Link key={`${item.label}-${index}`} href={item.href} className={`${styles.crmNavItem} ${active ? styles.crmNavItemActive : ""}`} onClick={() => setMenuOpen(false)}>
          ...
          <span className={styles.crmNavLabel}>System</span>
          <Link href={automationsHref} className={`${styles.crmNavItem} ${automationsActive ? styles.crmNavItemActive : ""}`} onClick={() => setMenuOpen(false)}>
            <span className={styles.navDot} />Automations
          </Link>
          <span className={`${styles.crmNavItem} ${styles.crmNavItemMuted}`}><span className={styles.navDot} />Settings<small>Soon</small></span>
        </nav>

        <div className={styles.crmSidebarFooter}>
          <span className={styles.healthDot} />
          <div><strong>Private workspace</strong><span>My Drip Nurse only</span></div>
        </div>
      </aside>
      ...
      <section className={styles.crmWorkspace}>
        <header className={styles.crmTopbar}>
          <div className={styles.crmTopbarTitle}>
            ...
            <div><span>My Drip Nurse</span><strong>{title}</strong></div>
          </div>
            ...
            <div className={styles.crmProfile} ref={profileRef}>
              ...
              {profileOpen ? (
                <div className={styles.profileMenu} role="menu">
                  <button type="button" role="menuitem" onClick={openProfileEditor}><span>✎</span><span><strong>Edit profile</strong><small>Name and profile photo</small></span></button>
                  <PartnerAdminLogout className={styles.profileSignOut} />
                </div>
              ) : null}
            </div>
          </div>
