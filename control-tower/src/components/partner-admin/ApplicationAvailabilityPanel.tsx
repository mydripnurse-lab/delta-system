...
export function ApplicationAvailabilityPanel({
  applicationId,
  accountActivated,
  onSaved,
}: {
  ...
  async function save() {
    ...
    try {
      ...
      setAvailability(payload.availability);
      setNotice(accountActivated
        ? "Weekly availability saved. New matching times are now available for booking."
        : "Weekly availability saved. This Partner can now receive matching bookings before activating their Portal account.");
      await onSaved?.();
    } catch (saveError) {
      ...
      <div className={styles.panelHeader}>
        <div className={styles.partnerCalendarHeading}>
          <div>
            ...
            <span className={styles.subtle}>Set the hours patients can book across every active service and approved coverage area.</span>
          </div>
          <span className={`${styles.badge} ${styles.info}`}>{accountActivated ? "Partner Portal active" : "Admin-managed schedule"}</span>
        </div>
      </div>
      ...
      {!loading && availability ? (
        <div className={styles.availabilityEditor}>
          ...
          <div className={styles.availabilityActions}>
            <span>
              {accountActivated
                ? "The Partner can also update these hours from their Portal. Coverage, service access and the 2-hour minimum notice still apply."
                : "Portal activation is not required for these hours to appear in the calendar. Coverage, service access and the 2-hour minimum notice still apply."}
            </span>
