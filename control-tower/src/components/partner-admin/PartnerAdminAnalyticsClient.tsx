...
type AnalyticsPayload = {
  ...
  managerFinancials: null | { completedAppointments: number; grossCompletedValue: number; grossAppointmentValue: number; earnedCommission: number; paidCommission: number; pendingCommission: number };
  ...
  return <PartnerAdminShell title="Business Analytics" actions={<button type="button" className={styles.secondaryButton} onClick={() => void load()} disabled={loading}>{loading ? "Refreshing…" : "Refresh data"}</button>}>
    <div className={styles.frame}>
      <section className={`${styles.moduleHeader} ${styles.analyticsHero}`}>
        <div><span className={styles.eyebrow}>{analytics && !analytics.viewer.isOwner ? "Your state workspace" : "Growth intelligence"}</span><h1>Business Analytics</h1><p>{analytics && !analytics.viewer.isOwner ? `Every KPI and market signal in this workspace is limited to ${analytics.viewer.stateNames.join(" and ") || "your assigned states"}.` : "Connect demand, completed visits, lost opportunities and active coverage to understand where the business can grow."}</p></div>
      ...
      {analytics ? <>
        <section className={styles.analyticsKpis}>
          ...
          <article><span>Intent opportunity value</span><strong>{money(summary?.intentOpportunityValue || 0)}</strong>{analytics.viewer.isOwner ? <div className={styles.lostKpiBreakdown}><div><small>My Drip Nurse</small><b>{money(summary?.intentPlatformRevenue || 0)}</b></div><div><small>Partners</small><b>{money(summary?.intentPartnerEarnings || 0)}</b></div></div> : null}<small>{summary?.appointmentIntents || 0} unique people · {summary?.bookingAttempts || 0} booking attempts</small></article>
          <article className={styles.lostKpi}>
            <span>Lost opportunity value</span>
            <strong>{money(summary?.lostOpportunityValue || 0)}</strong>
            {analytics.viewer.isOwner ? <div className={styles.lostKpiBreakdown}>
          ...
          <article className={styles.partnerRevenueKpi}><span>Active pipeline value</span><strong>{money(summary?.activePipelineValue || 0)}</strong>{analytics.viewer.isOwner ? <div className={styles.lostKpiBreakdown}><div><small>My Drip Nurse</small><b>{money(summary?.activePlatformValue || 0)}</b></div><div><small>Partners</small><b>{money(summary?.activePartnerValue || 0)}</b></div></div> : null}<small>{summary?.active || 0} scheduled or in progress · {money(summary?.pendingDeposits || 0)} pending deposits</small></article>
          {analytics.viewer.isOwner ? <article className={styles.partnerRevenueKpi}><span>Generated for Partners</span><strong>{money(summary?.partnerEarnings || 0)}</strong><small>Service earnings from completed visits</small></article> : null}
          {analytics.viewer.isOwner ? <article className={styles.platformRevenueKpi}><span>My Drip Nurse collected</span><strong>{money(summary?.platformRevenue || 0)}</strong><div className={styles.lostKpiBreakdown}><div><small>Refunded</small><b>{money(summary?.refundedRevenue || 0)}</b></div><div><small>Failed</small><b>{money(summary?.failedDepositValue || 0)}</b></div></div><small>Net deposits collected across paid appointments</small></article> : null}
          ...
          {analytics.managerFinancials ? <article className={styles.managerEarningsKpi}><span>Your manager earnings</span><strong>{money(analytics.managerFinancials.earnedCommission)}</strong><div className={styles.lostKpiBreakdown}><div><small>Paid</small><b>{money(analytics.managerFinancials.paidCommission)}</b></div><div><small>Payable</small><b>{money(analytics.managerFinancials.pendingCommission)}</b></div></div><small>Calculated independently from completed appointments in each assigned state.</small></article> : null}
        </section>
        ...
        <section className={styles.analyticsMapPanel}>
          ...
          <AppointmentAnalyticsMap points={analytics.points} people={analytics.people || []} coverageAreas={analytics.coverageAreas || []} coverageVisible={coverageVisible} coverageGapsVisible={coverageGapsVisible} focusStateCodes={analytics.viewer.isOwner ? [] : analytics.viewer.stateCodes} />
        </section>
