...
export default async function ClientVisitRewardsPage() {
  ...
  const summary = await getClientVisitRewardSummary(account.id);
  const rewardTitle = summary.availableRewards
    ? `${summary.availableRewards} free visit reward${summary.availableRewards === 1 ? " is" : "s are"} ready.`
    : `${summary.remainingVisits} completed visit${summary.remainingVisits === 1 ? "" : "s"} to your next reward.`;
    ...
    <div className={`${styles.pageShell} ${styles.visitRewardsPage}`}>
      ...
      <section className={styles.visitRewardHero}>
        <div className={styles.visitRewardCopy}>
          <span className={styles.eyebrow}>Wellness visits</span>
          <h1>Your care builds toward more care.</h1>
          <p>Complete {summary.goal} eligible My Drip Nurse appointments to unlock one free eligible visit. You pay no deposit or appointment balance, and your next cycle continues after the reward is used.</p>
      ...
      <section className={styles.visitRewardDetails}>
        <article><span>01</span><div><small>Keep it simple</small><h2>Complete your appointment.</h2><p>Only completed visits count. Cancelled or unfinished appointments do not change your progress.</p></div></article>
        <article><span>02</span><div><small>Automatic progress</small><h2>We track every milestone.</h2><p>Your progress updates automatically after the professional completes the visit.</p></div></article>
        <article><span>03</span><div><small>Ready when you are</small><h2>Use your free visit.</h2><p>The earned reward covers your next eligible appointment in full. My Drip Nurse funds your care professional’s normal payment after the visit is completed.</p></div></article>
      </section>

      <section className={styles.visitRewardCta}>
        <div><span className={styles.eyebrow}>{summary.availableRewards ? "Reward ready" : `Next milestone · ${summary.nextMilestone} completed visits`}</span><h2>{summary.availableRewards ? "Choose your next wellness moment." : "Keep your wellness rhythm going."}</h2></div>
