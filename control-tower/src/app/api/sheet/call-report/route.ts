...
function normalizeState(raw: any) {
    ...
    // (keep exact title-case from map if possible)
...
export async function GET(req: Request) {
    try {
        ...
        let objects = rowsToObjects(headers, rows).map((r: any) => {
            ...
            const startIso = d ? d.toISOString() : "";
            const startMs2 = d ? d.getTime() : null;

            const stNorm = normalizeState(r["Phone Call From State"]);

            return {
                ...r,
                __startIso: startIso,
                __startMs: startMs2,
                __fromStateNorm: stNorm,
            };
        });
