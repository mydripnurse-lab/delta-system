...
export async function POST(req: Request) {
  try {
    ...
    const primaryCountyKey = s(body?.primaryCountyKey);
    ...
    if (isMultipart) {
      ...
      if (input.professionalCredentials.length > 100) {
        return NextResponse.json({ error: "Licenses and professional credentials must be 100 characters or fewer" }, { status: 400, headers: cors });
      }
    ...
    const primaryCounty = selected.length === 1
      ? selected[0]
      : selected.find((county) => county.key === submittedPrimaryCounty || county.locationId === submittedPrimaryCounty);
    if (!primaryCounty) {
      return NextResponse.json({ error: "Choose a valid primary county from the selected coverage areas" }, { status: 400, headers: cors });
    }
    input.primaryLocationId = primaryCounty.locationId;
    if (isMultipart) {
