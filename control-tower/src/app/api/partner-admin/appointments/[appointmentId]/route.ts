...
import { sendAdminPartnerAppointmentReminder, type PartnerReminderAction } from "@/lib/partnerPushNotifications";
  ...
  try {
    ...
    if (body.action === "remind") {
      if (!body.reminderAction || !["accept", "start", "complete"].includes(body.reminderAction)) {
        return NextResponse.json({ ok: false, error: "Choose a valid appointment reminder." }, { status: 400 });
      }
      const result = await sendAdminPartnerAppointmentReminder({
        appointmentId,
        action: body.reminderAction,
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Could not update this appointment." }, { status: 400 });
  }
}
