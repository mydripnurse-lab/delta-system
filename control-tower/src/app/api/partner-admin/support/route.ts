...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, tickets: await listAdminSupportTickets(organizationId), agents: await listSupportAgents() }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const organizationId = organization.rows[0]?.id;
    if (!organizationId) return NextResponse.json({ ok: false, error: "Administrator organization not found." }, { status: 404 });
    await updateAdminSupportTicket({ ticketId, organizationId, adminUserId: auth.user.id, body: message || undefined, status: status as "open" | "pending" | "closed" | undefined, assignedUserId });
    return NextResponse.json({ ok: true, tickets: await listAdminSupportTickets(organizationId), agents: await listSupportAgents() });
  } catch (error) {
