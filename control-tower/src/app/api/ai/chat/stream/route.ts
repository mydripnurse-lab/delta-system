        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    controller.enqueue(
                        encoder.encode(
                            sse("meta", { agent, threadId, startedAt: Date.now() }),
                        ),
                    );
                    const responseStream = await (client as any).responses.stream({
                        model: "gpt-5.2",
                    ...
                    for await (const event of responseStream) {
                        ...
                        const delta = s(
                            ev?.delta ||
                                ev?.text ||
                                (String(ev?.type || "").includes("output_text.delta") ? ev?.delta : ""),
                        );
                        if (delta) {
                            full += delta;
                    ...
                    if (!full) {
                        const finalText = s((await responseStream.finalResponse())?.output_text || "");
                        full = finalText;
                    }
                    full = s(full);
                    if (!full) {
                        controller.enqueue(encoder.encode(sse("error", { error: "Empty model output." })));
                        controller.close();
                        return;
                    }
