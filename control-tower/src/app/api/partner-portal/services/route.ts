...
import { listPartnerPortalServices, setPartnerPortalService, type PartnerServiceAssignmentStatus } from "@/lib/partnerServiceAssignments";
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const status = String(body?.status || "").trim().toLowerCase() as PartnerServiceAssignmentStatus;
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(serviceKey) || !["active", "paused", "out_of_stock"].includes(status)) {
      return NextResponse.json({ ok: false, error: "A valid service and availability status are required." }, { status: 400 });
    }
    const matrix = await setPartnerPortalService({ applicationId: session.application_id, serviceKey, status });
