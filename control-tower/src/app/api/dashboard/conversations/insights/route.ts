    ...
    try {
        ...
        const resp = await client.responses.create({
            ...
            text: {
                format: {
                    type: "json_schema",
                    name: "conversations_dashboard_insights",
                    schema,
                },
            },
        });
        ...
        await appendAiEvent({
            agent: "conversations",
            kind: "insight_run",
            summary: `Conversations insights generated (${String(insights?.scorecard?.health || "mixed")})`,
            metadata: {
        ...
        return NextResponse.json(
            { ok: false, error: e?.message || "Failed to generate conversations insights" },
            { status: 500 },
        );
    }
}
