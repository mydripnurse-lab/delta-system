...
export async function updatePartnerPortalProfile(opts: {
  ...
  const result = await getDbPool().query<ProfileRow>(
    `update app.partner_profiles
        set public_title = $2,
            professional_credentials = nullif($3, ''),
            biography = $4,
            profile_photo_url = coalesce(nullif($5, ''), profile_photo_url),
            profile_photo_data = coalesce(nullif($8, ''), profile_photo_data),
            profile_photo_content_type = coalesce(nullif($9, ''), profile_photo_content_type),
            profile_photo_file_id = coalesce(nullif($6, ''), profile_photo_file_id),
            profile_photo_location_id = coalesce(nullif($7, ''), profile_photo_location_id),
            updated_at = now()
      where id = $1
      returning id`,
    [
