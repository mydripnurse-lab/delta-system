...
export async function refundAdminBookingAppointment(opts: {
  ...
  const pool = getDbPool();
  const client = await pool.connect();
  let payment: { paymentIntentId: string; amount: number; currency: string } | null = null;
  try {
    await client.query("begin");
    const locked = await client.query<{
      id: string;
    ...
    payment = {
      ...
      amount: number(appointment.payment_amount),
      currency: text(appointment.payment_currency) || "USD",
    };
    await client.query(
