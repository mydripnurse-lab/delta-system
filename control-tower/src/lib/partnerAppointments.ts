...
import { applyStripeRefundUpdate } from "@/lib/stripePaymentLifecycle";
...
export type PartnerPortalAppointment = {
  ...
  platformFundedReward: boolean;
  platformFundedAmount: number;
  currency: string;
};

function calculateBmi(weightValue: unknown, heightValue: unknown): number | null {
...
export async function listPartnerPortalAppointments(profileId: string): Promise<PartnerPortalAppointment[]> {
  ...
  const result = await getDbPool().query<{
    ...
    email: string;
    phone: string;
    metadata: { primary_patient?: { dateOfBirth?: string; weight?: string; height?: string }; additional_patients?: Array<{ fullName?: string; email?: string; phone?: string; dateOfBirth?: string }> } | null;
    ...
    platform_funded_reward: boolean;
    platform_funded_amount: string;
    currency: string;
  }>(
    `select appointment.id, appointment.public_reference,
            service.name as service_name,
            service.image_url as service_image_url,
            service.image_alt as service_image_alt,
            ...
            case
              when coalesce((appointment.metadata #>> '{client_reward,platformFunded}')::boolean, false) then '0'
              else greatest(appointment.service_price - appointment.deposit_amount, 0)::text
            end as amount_due_at_visit,
            greatest(appointment.service_price - appointment.deposit_amount, 0)::text as partner_earnings,
            coalesce((appointment.metadata #>> '{client_reward,platformFunded}')::boolean, false) as platform_funded_reward,
            coalesce(
              nullif(appointment.metadata #>> '{client_reward,platformFundedPartnerAmount}', '')::numeric,
              greatest(appointment.service_price - appointment.deposit_amount, 0)
            )::text as platform_funded_amount,
...
`,
  ...
  return result.rows.map((row) => ({
    ...
    serviceImageAlt: row.service_image_alt || row.service_name,
    ...
    customerWeight: row.metadata?.primary_patient?.weight || "",
    customerHeight: row.metadata?.primary_patient?.height || "",
    customerBmi: calculateBmi(
      row.metadata?.primary_patient?.weight,
      row.metadata?.primary_patient?.height,
    ),
    ...
    partnerEarnings: Number(row.partner_earnings || 0),
    platformFundedReward: row.platform_funded_reward,
    platformFundedAmount: Number(row.platform_funded_amount || 0),
...
export async function advancePartnerAppointment(opts: {
  ...
  try {
    ...
    const lockedAppointment = await client.query<{ starts_at: string; status: string; metadata: unknown }>(
      `select starts_at::text, status, metadata
    ...
    const appointmentMetadata = lockedRow.metadata && typeof lockedRow.metadata === "object" && !Array.isArray(lockedRow.metadata)
      ? lockedRow.metadata as Record<string, unknown>
      : {};
    const clientReward = appointmentMetadata.client_reward && typeof appointmentMetadata.client_reward === "object" && !Array.isArray(appointmentMetadata.client_reward)
      ? appointmentMetadata.client_reward as Record<string, unknown>
      : {};
    const platformFundedReward = clientReward.platformFunded === true;
    const platformFundedAmount = Number(clientReward.platformFundedPartnerAmount || 0);
    const fundingPayload = platformFundedReward ? {
      platformFunded: true,
      paymentSource: "my_drip_nurse",
      platformFundedPartnerAmount: Number.isFinite(platformFundedAmount) ? platformFundedAmount : 0,
      clientCollectionAmount: 0,
      platformFundingStatus: opts.action === "complete" ? "payment_due" : "owed_after_completion",
    } : {};
    const standardStartWindowMs = 60 * 60 * 1000;
    if (opts.action === "start" && new Date(lockedRow.starts_at).getTime() > Date.now() + standardStartWindowMs && earlyStartReason.length < 3) {
      throw new Error("This visit can start normally up to 1 hour before its scheduled time. Earlier starts require a reason.");
    }
    ...
    await client.query(
      ...
      [
        ...
        opts.profileId,
        JSON.stringify({
          ...(opts.action === "start" && earlyStartReason
    ...
    if (transition.to === "completed" && platformFundedReward) {
      await client.query(
        `insert into app.appointment_events (appointment_id, event_type, actor_type, payload)
         values ($1, 'platform_funded_partner_payment_due', 'system', $2::jsonb)`,
    ...
    const notificationTypes = opts.action === "acknowledge"
      ? ["appointment_confirmation", "appointment_reassigned"]
      : opts.action === "start" ? ["start_reminder"] : ["complete_reminder"];
    await pool.query(
      `update app.partner_portal_notifications
          set read_at = coalesce(read_at, now())
        where partner_profile_id = $1::uuid
          and appointment_id = $2::uuid
          and event_type = any($3::text[])`,
      [opts.profileId, opts.appointmentId, notificationTypes],
    ).catch(() => undefined);
  } catch (error) {
...
export async function declinePartnerAppointment(opts: {
  ...
  let payment: { paymentIntentId: string | null; amount: number; currency: string; status: string } | null = null;
  ...
  try {
    ...
    const locked = await client.query<{
      ...
      payment_intent_id: string | null; payment_amount: string | null; payment_currency: string | null; payment_status: string | null;
    }>(
      `select appointment.id, appointment.organization_id, appointment.service_id, appointment.partner_profile_id,
              ...
              payment.payment_intent_id, payment.amount::text as payment_amount,
              payment.currency as payment_currency, payment.status as payment_status
         from app.appointments appointment
    ...
    if (replacement) {
      ...
      if (payment?.status === "paid") {
        await client.query(
          `update app.appointment_payments set status = 'processing', updated_at = now()
  ...
  try {
    ...
    await pool.query(
      `update app.appointment_payments set status = 'paid', updated_at = now()
  ...
  const applied = await applyStripeRefundUpdate({
  ...
  return {
    outcome: applied.fullyRefunded ? "refunded" as const : "refund_pending" as const,
    replacementPartnerName: null,
    appointments: await listPartnerPortalAppointments(opts.profileId),
  };
}
