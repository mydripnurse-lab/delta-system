...
import { activatePartnerOnboardingToken, readPartnerOnboardingToken } from "@/lib/partnerOnboarding";
...
export async function activatePartnerPortalAccount(onboardingToken: string, passwordRaw: string) {
  const password = passwordRaw.trim();
  const passwordError = validatePasswordStrength(password);
  if (passwordError) throw new Error(passwordError);
  await ensurePortalSessionSchema();
  const profile = await activatePartnerOnboardingToken(onboardingToken, await hashPassword(password));
