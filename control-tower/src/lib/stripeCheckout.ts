...
export async function createStripeCheckoutSession(opts: {
  ...
  const bookingBaseUrl = String(process.env.BOOKING_PUBLIC_BASE_URL || "https://admin.mydripnurse.com")
    .trim()
    .replace(/\/+$/, "");
  ...
  form.set("cancel_url", `${bookingBaseUrl}/booking/${encodeURIComponent(opts.calendarPublicKey)}?payment=cancelled`);
...
export async function refundStripePayment(opts: { paymentIntentId: string; amountCents: number; reason?: string }) {
  const secretKey = requiredEnvironment("STRIPE_SECRET_KEY");
  ...
  const response = await fetch("https://api.stripe.com/v1/refunds", {
    ...
    headers: {
      Authorization: `Bearer ${secretKey}`,
      "Content-Type": "application/x-www-form-urlencoded",
      "Idempotency-Key": `mdn-appointment-refund-${opts.paymentIntentId}`,
    },
    body: form,
    signal: AbortSignal.timeout(20_000),
  });
  const payload = await response.json() as { id?: string; status?: string; error?: { message?: string } };
  if (!response.ok || !payload.id || !["succeeded", "pending"].includes(payload.status || "")) {
