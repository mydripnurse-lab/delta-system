...
async function syncProfileServiceAssignments(profile: ProfileRow, client: Pool | PoolClient) {
  ...
  for (const profileService of configured) {
    ...
    const assignmentStatus: PartnerServiceAssignmentStatus = text(profileService.status).toLowerCase() === "out_of_stock" ? "out_of_stock" : "active";
    ...
    const assignment = await client.query<{ id: string }>(
      `insert into app.partner_service_assignments (
       ...
       ) values ($1, $2, $3, $4, $5, case when $4 = 'active' then now() end, '{"source":"profile_sync"}'::jsonb)
       on conflict (partner_profile_id, service_id) do update set
         ...
         activated_at = case when excluded.status = 'active' then coalesce(app.partner_service_assignments.activated_at, now()) else app.partner_service_assignments.activated_at end,
         deactivated_at = case when excluded.status = 'active' then null else now() end,
         updated_at = now()
       returning id`,
      [profile.organization_id, profile.id, service.id, assignmentStatus, priceOverride],
    );
...
export async function listPartnerPortalServices(applicationId: string) {
  ...
  return rows.map((row) => ({
    ...
    publicKey: row.public_key,
    calendarStatus: row.calendar_status,
    status: row.assignment_status === "out_of_stock" ? "out_of_stock" : row.assignment_status === "active" ? "active" : "paused",
    active: row.assignment_status === "active",
    outOfStock: row.assignment_status === "out_of_stock",
  }));
}

export async function setPartnerPortalService(opts: { applicationId: string; serviceKey: string; status: PartnerServiceAssignmentStatus }) {
...
export async function listPartnerAdminDirectory(): Promise<PartnerAdminDirectoryItem[]> {
  ...
  const result = await getDbPool().query<{
    ...
    `select p.id, p.application_id, p.display_name, p.email,
            p.business_name, p.profile_photo_url, p.slug, p.website_status, p.directory_status,
  ...
  return result.rows.map((row) => ({
    ...
    businessName: text(row.business_name),
    profilePhotoUrl: text(row.profile_photo_url),
    websiteUrl: `${websiteBase}/${row.slug}`,
    websitePreviewUrl: `${websiteBase}/${row.slug}?preview=${encodeURIComponent(row.application_id)}`,
    websiteStatus: row.website_status,
...
export async function setPartnerServiceAssignment(opts: {
  ...
  serviceKey: string;
  active?: boolean;
  status?: PartnerServiceAssignmentStatus;
}) {
  const pool = getDbPool();
  ...
  try {
    ...
    const nextStatus: PartnerServiceAssignmentStatus = opts.status || (opts.active ? "active" : "paused");
    if (nextStatus === "active" && service.calendar_status !== "active") {
    ...
    const assignment = await client.query<{ id: string }>(
      `insert into app.partner_service_assignments (
       ...
       on conflict (partner_profile_id, service_id) do update set
         ...
         updated_at = now()
       returning id`,
      [profile.organization_id, profile.id, service.id, nextStatus],
    );
    const assignmentId = assignment.rows[0]?.id;
