...
export type PartnerCountyCoverage = PartnerMapPoint & {
  geometry: {
...
type TigerGeoJsonResponse = {
  features?: Array<{
    geometry?: PartnerCountyCoverage["geometry"];
    properties?: { CENTLAT?: string; CENTLON?: string };
  }>;
};
...
export const resolveCountyBoundary = cache(async function resolveCountyBoundary(
  area: PartnerServiceArea,
): Promise<PartnerCountyCoverage | null> {
  const state = clean(area.state);
  const county = clean(area.county);
  const stateFips = STATE_FIPS[state.toLowerCase()];
  if (!stateFips || !county) return null;
  ...
  try {
    const response = await fetch(
      `https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/State_County/MapServer/19/query?${params.toString()}`,
      { headers: { Accept: "application/geo+json,application/json" }, next: { revalidate: 60 * 60 * 24 * 30 } },
    );
    if (!response.ok) return null;
    const body = await response.json() as TigerGeoJsonResponse;
    const feature = body.features?.[0];
    const latitude = Number(feature?.properties?.CENTLAT);
    const longitude = Number(feature?.properties?.CENTLON);
    if (!feature?.geometry || !Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
    return { ...area, latitude, longitude, geometry: feature.geometry };
  } catch {
    return null;
  }
});
